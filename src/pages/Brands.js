...
const Brands = () => {
  ...
  const fetchCategories = async () => {
    try {
      const { data } = await axios.get(`${API_URL}/categories?type=brand`);
      setCategories(data);
    } catch (error) {
  ...
  const handleAddSubmit = async (e) => {
    e.preventDefault();
    try {
      const categoryData = { name, iconImage, type: 'brand' };
      ...
      console.error('Error saving brand/category:', error.response?.data || error.message);
      alert(`Failed to save brand/category. ${error.response?.data?.message || ''}`);
    }
  };
    ...
    <div className="bg-white p-6 rounded shadow">
      ...
      {isAdding && (
        <div className="mb-8 p-6 border rounded bg-gray-50">
          <h4 className="text-lg font-bold mb-4">{editId ? 'Edit Brand' : 'Create New Brand'}</h4>
          <form onSubmit={handleAddSubmit} className="space-y-6">
            ...
            <div>
              <button type="submit" className="w-full bg-green-600 text-white px-6 py-3 text-lg font-bold rounded hover:bg-green-700">Save Brand</button>
            </div>
          </form>
        </div>
      )}
      ...
      <table className="w-full text-left border-collapse">
        ...
        <tbody>
          {categories.length === 0 ? (
            <tr><td colSpan="4" className="p-3 text-center text-gray-500">No brands found.</td></tr>
          ) : (
            categories.map((c) => (
